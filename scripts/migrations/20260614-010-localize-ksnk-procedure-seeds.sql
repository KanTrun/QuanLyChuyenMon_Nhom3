...
('f0000000-0000-0000-0000-000000000009','f1000000-0000-0000-0000-000000000009',N'QT.KSNK.09',N'Quy trình xử lý dụng cụ phẫu thuật',N'2145_QUY TRÌNH XỬ LÝ DỤNG CỤ PHẪU THUẬT.pdf'),
('f0000000-0000-0000-0000-000000000012','f1000000-0000-0000-0000-000000000012',N'QT.KSNK.12',N'Quy trình xử lý dụng cụ y tế',N'2145_QUY TRÌNH XỬ LÝ DỤNG CỤ Y TẾ.pdf'),
('f0000000-0000-0000-0000-000000000016','f1000000-0000-0000-0000-000000000016',N'QT.KSNK.16',N'Quy trình khử khuẩn mức độ cao dụng cụ y tế',N'2145_QUY TRÌNH KHỬ KHUẨN MỨC ĐỘ CAO DỤNG CỤ Y TẾ.pdf'),
('f0000000-0000-0000-0000-000000000017','f1000000-0000-0000-0000-000000000017',N'QT.KSNK.17',N'Quy trình xử lý tay khoan nha khoa',N'2145_QUY TRÌNH XỬ LÝ TAY KHOAN NHA KHOA.pdf');

UPDATE p
SET p.name = s.title,
    p.description = N'Nhập từ PDF scan 2145; đang chờ OCR đầy đủ từng trang trước khi ban hành.',
    p.updated_at = SYSUTCDATETIME()
FROM med.professional_procedures p
JOIN @seed s ON s.procedure_id = p.procedure_id;

UPDATE v
SET v.version_label = N'v01',
    v.title = s.title,
    v.summary = N'{"ocrStatus":"OCR_PENDING","note":"PDF scan là nguồn sự thật; chỉ seed metadata và lưu đồ đã spot-check từ ảnh render."}',
    v.change_reason = N'Nhập quy trình KSNK từ PDF scan',
    v.source_pdf_file_name = s.file_name
FROM med.procedure_versions v
JOIN @seed s ON s.version_id = v.procedure_version_id
...
JOIN @seed seed ON seed.version_id = d.procedure_version_id
...
JOIN @seed s ON s.version_id = r.procedure_version_id;
...
UPDATE p
SET p.name = f.name,
    p.description = N'OCR_PENDING: diễn giải chi tiết cần trích xuất và đối chiếu từ từng trang PDF scan.',
    p.responsibility_text = CASE WHEN p.step_no = 1 THEN N'Khoa sử dụng / Khoa KSNK' ELSE N'Khoa KSNK' END
FROM med.procedure_steps p
JOIN @seed s ON s.version_id = p.procedure_version_id
JOIN @flow f ON f.code = s.code AND f.step_no = p.step_no;

UPDATE a
SET a.file_name = s.file_name,
    a.file_uri = N'imported/' + s.file_name
FROM med.procedure_attachments a
JOIN @seed s ON s.version_id = a.procedure_version_id
