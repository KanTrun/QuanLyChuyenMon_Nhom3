...
public sealed class ClinicalExportService(IMedDataStore store) : IClinicalExportService
{
        ...
        bool professionalLabels = false)
    {
        ...
        {
            foreach (var app in apps.Where(a => a.ApplicationStatus is "signed" or "revoked"))
            {
    ...
    {
        var signature = store.SignatureRecords.FirstOrDefault(s =>
            s.TargetType == SignatureTargetType &&
            s.TargetId == app.PatientProtocolApplicationId);
        ...
        html.AppendLine("</tbody></table>");
        if (SignatureService.HasValidIntegrity(signature) &&
            TryReadSignatureImageDataUrl(signature.MetadataJson, out var imageDataUrl))
        {
