...
public sealed class ProcedureDocumentExportService : IProcedureDocumentExportService
{
    ...
    private const int FlowStepsPerPage = 9;
...
""";
        ...
        DateTime generatedAt)
    {
        ...
        {
            var chunks = SplitContent(PrintableText(section.ContentText), SectionPageCharacterLimit);
...
<tr class="flow-table-row">
  ...
  <td class="flow-step-cell">
    <div class="flow-symbol shape-{{Shape(step.FlowShapeCode)}}"><span>{{H(PrintableText(step.Name, "Chưa đặt tên bước"))}}</span></div>
    {{(index < steps.Count - 1 ? "<div class=\"flow-arrow\">↓</div>" : "")}}
  </td>
