...
<table><thead><tr><th>STT</th><th>Tên tệp</th><th>Loại</th><th>Dung lượng</th><th>SHA-256</th></tr></thead><tbody>{{AttachmentRows(attachments)}}</tbody></table>
<h2 class="block-title">Nhật ký chữ ký nội bộ</h2>
<table class="signoff-log"><thead><tr><th>STT</th><th>Vai trò</th><th>Người xác nhận</th><th>Thời điểm</th><th>Hash nội dung</th><th>Hiệu lực</th></tr></thead><tbody>{{SignoffRows(signoffs, hash)}}</tbody></table>
...
""";
    ...
    private static string AttachmentRows(IEnumerable<ProcedureAttachment> attachments)
    {
        var rows = attachments
            .Select((item, index) => $"<tr><td>{index + 1}</td><td>{H(item.FileName)}</td><td>{H(AttachmentLabel(item.AttachmentType))}</td><td>{FileSize(item.FileSizeBytes)}</td><td class=\"hash\">{H(item.ChecksumSha256)}</td></tr>")
            .ToList();
        return rows.Count == 0
    ...
    private static string SignoffRows(IEnumerable<ProcedureSignoffRecord> signoffs, string hash)
    {
        var rows = signoffs.Select((sign, index) =>
        {
            ...
            return $"<tr><td>{index + 1}</td><td>{H(RoleLabel(sign.SignoffRole))}</td><td>{H(sign.SignerFullName ?? sign.SignerUsername)}</td><td>{D(sign.SignedAt)}</td><td class=\"hash\">{H(sign.ContentHashSha256)}</td><td><span class=\"sign-state {(isCurrent ? "is-current" : "is-stale")}\">{(isCurrent ? "Còn hiệu lực" : "Hết hiệu lực")}</span></td></tr>";
        }).ToList();
        return rows.Count == 0
            ? "<tr><td colspan=\"6\" class=\"empty\">Chưa có xác nhận nội bộ</td></tr>"
            : string.Join("", rows);
    }
...
""";
    ...
    private static string SignCard(ProcedureDocumentSnapshot s, string role, string label, string currentHash)
    {
        ...
        return $"<div class=\"signature-card\"><h3>{label}</h3>{image}<strong>{H(sign.SignerFullName ?? sign.SignerUsername)}</strong><span class=\"sign-account\">Tài khoản: {H(sign.SignerUsername)}</span><span>{D(sign.SignedAt)}</span><span class=\"hash short-hash\">{H(sign.ContentHashSha256[..Math.Min(16, sign.ContentHashSha256.Length)])}…</span></div>";
    }
