...
| VNPT SmartCA sandbox signing | Credential pending | SmartCA SP sandbox client, signer binding, pending transaction table, clinical signing UI, Docker API endpoints, callback endpoint, certificate evidence export, Docker env mapping and local env configurator are implemented; live VNPT signing still needs sandbox SP credential/binding |
| Seed data | Complete | `scripts/seed-realistic-data.sql` nạp dữ liệu mẫu thực tế cho demo/QA |
| Verification | Complete | Release build `0 warnings, 0 errors`; full solution `228/228`; Docker web healthy; SmartCA callback without secret returns `403`; Docker Compose config valid |
...
| Test automation | Release build `0 warnings, 0 errors`; full solution `228/228`; package vulnerability scan clean |
| Deployment readiness | Docker Compose config valid; web/sql/db-init verified locally; SmartCA API routes available from Docker web; live SmartCA signing waits for VNPT sandbox credential/binding; re-check stable Gemini model before production rollout |
...
## Next Milestone
Test one live VNPT SmartCA sandbox signature after SP credential, subscriber id and signer binding are supplied. Then run browser QA by role on target SQL Server: `SYSTEM_ADMIN`, `DEPARTMENT_ADMIN`, clinical user, technician/pharmacist and report viewer.
